...
GFX_LoadDataTexture2D(u8* data, V2I size, GFX_ColorFormat format)
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture.id);
    {
        ...
        switch (format)
        {
            ...
            case GFX_Format_RGB:
            {
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texture.size.w, texture.size.h, 0, GL_RGB, GL_UNSIGNED_BYTE, texture.data);
            } break;
            
            case GFX_Format_BGR:
            {
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texture.size.w, texture.size.h, 0, GL_BGR, GL_UNSIGNED_BYTE, texture.data);
            } break;
...
void GFX_UpdateTexture2D(GFX_Texture2D* texture, u8* data, GFX_ColorFormat format)
{
    glBindTexture(GL_TEXTURE_2D, texture->id);
    ...
    if (format == GFX_Format_RGBA) gl_format = GL_RGBA;
    if (format == GFX_Format_BGR)  gl_format = GL_BGR;
