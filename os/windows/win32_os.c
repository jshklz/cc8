...
OS_GetTime(OS_App* app)
{
    ...
    return ((( 1000* time.QuadPart) / freq.QuadPart) - handle->time_start);
...
OS_ReadFile(const u8* path)
{
    ...
    char* read_buffer = MEM_ArenaAlloc(MEM_GetDefaultArena(), bytes + 1);
    DWORD bytes_read = 0;
...
OS_GetKeyState(i8 key)
{
    ...
    if (key >= 'a' && key <= 'z') {
        key -= 32; // convert to A-Z for VK
    }
    
    SHORT vk_state = GetAsyncKeyState(key );
    state.pressed = (vk_state & 0x8000);
